...
// mqtt
#include <AsyncMqttClient.h>  // https://github.com/marvinroger/async-mqtt-client v0.9.0
// ota
...
void setColorAllPixelsRed(byte red) {
  char msg[40];
  snprintf(msg, 100, "setColorAllPixelsRed: %u", red);
  debug(msg);
...
void setColorAllPixelsGreen(byte green) {
  char msg[40];
  snprintf(msg, 100, "setColorAllPixelsGreen: %u", green);
  debug(msg);
...
void setColorAllPixelsBlue(byte blue) {
  char msg[40];
  snprintf(msg, 100, "setColorAllPixelsBlue: %u", blue);
  debug(msg);
...
void LEDStrip_smoothly_match(uint8_t target, uint8_t *current) {
  uint16_t target16 = (uint16_t)(target) & 0x00FF;
  uint16_t current16 = (uint16_t)(*current) & 0x00FF;
  ...
  if (current16 > 0x00FF) {
    char msg[200];
    snprintf(msg, 200, "LEDStrip_smoothly_match: weird %u, %u, %u, %u", *current, current16, target, target16);
    debug(msg);
    current16 = 0x00FF;
  }
...
void LEDStrip_Task(void *pvParameters) {
  ...
  while (1) {
    for (i = 0; i < LED_STRIP_LED_COUNT; i++) {
      ...
      if ((pixels_raw[i].r != red) || (pixels_raw[i].g != green) || (pixels_raw[i].b != blue)) {
        //snprintf(msg, 200, "task: LED %u: %u (%u), %u (%u), %u, (%u)", i, pixels_raw[i].r, red, pixels_raw[i].g, green, pixels_raw[i].b, blue);
        //debug(msg);
        ...
        led_strip.setPixelColor(i, pixels_raw[i].r, pixels_raw[i].g, pixels_raw[i].b);
      }
    }
